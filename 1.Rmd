...
for (i in seq_along(archivos)) {
  variables[i] <- paste0("vec",i)
  data <- read_delim(paste0("data/",archivos[i]),delim = ";")
  assign(variables[i],data)
}
```
...
```{r}
cabecera <- left_join(vec1,vec8, by = c("DIRECTORIO","SECUENCIA_P"))

for (i in 2:7) {
  cabecera <- left_join(cabecera,get(paste0("vec",i)),by = c("DIRECTORIO","SECUENCIA_P","ORDEN"))
}
head(cabecera)
```

```{r}
resto <- left_join(vec9,vec16, by = c("DIRECTORIO","SECUENCIA_P"))

for (i in 10:15) {
  resto <- left_join(resto,get(paste0("vec",i)),by = c("DIRECTORIO","SECUENCIA_P","ORDEN"))
}
```

```{r}
geih_feb21 <- bind_rows(cabecera,resto)
```
...
```{r}
Covid_19 <- covid19 %>% 
  mutate(fecha_reporte1 = date(fecha_reporte)) %>%
...
ggplot(a,
  ...
  labs(title = "Colombia: Casos de Covid-19", 
       x = "Fecha", 
       y = "Número de casos") +
  theme_linedraw()
```
