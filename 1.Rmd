...
bm_ancha <- pivot_wider(union_bm1,
                  id_cols = c("indicator", "iso2c", "iso3c", "country", "date", "region", "income_level"),
...
#View(bm_ancha)
bm_ancha1 <- bm_ancha %>%
  select(indicator, iso2c, iso3c, country, date, region, income_level, ing_nac_ajustado, expectativa_vida) 
head(bm_ancha1)
#View(bm_ancha1)
```
...
```{r}
bm_ancha2 <- bm_ancha1 %>%
  select(region, income_level, country, date, ing_nac_ajustado, expectativa_vida, indicator, iso2c, iso3c)
#quitado variable indicator_id
head(bm_ancha2)
